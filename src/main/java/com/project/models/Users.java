...
@NamedNativeQuery(
        name="scoreboard",
        query = "Select users.username, sum(distinct(tasks.points)) as points from users inner join tasks_time on users.username = tasks_time.username inner join tasks on tasks_time.task_id = tasks.id group by users.username order by points desc limit 10",
